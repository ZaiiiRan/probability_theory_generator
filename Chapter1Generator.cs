...
namespace probability_theory_generator
{
    public static class Chapter1Generator
    {
        ...
        {
            TaskTemplate t = JSONReader.ReadJSON("Chapter1Task1.json");
            string s = "";
            for (int i = 0; i < t.Parametres.Count; i++)
            {
                s += t.Parametres[i].ToString();
            }
        }
        public static FinishedTask GenerateTask1()
        {
            ...
            double a = 0;
            double b = 0;
            Task solveA = new(() => a = (double) 1 / SuperMath.Factorial(X));
            Task solveB = new(() => b = (double) SuperMath.Factorial(X - (B - A + 1)) / SuperMath.Factorial(X));
            solveA.Start();
            solveB.Start();
            ...
            FinishedTask finishedTask = new(text, answer);
            return finishedTask;
        }
        public static FinishedTask GenerateTask2()
        {
            ...
            double a = 0, b = 0;
            Task solveA = new(() => a = (double) SuperMath.NumberOfCombinations(A + B, X) / SuperMath.NumberOfCombinations(A+B+C, X));
            Task solveB = new(() => b = (double) SuperMath.NumberOfCombinations(A, 1) * SuperMath.NumberOfCombinations(B, 1)
